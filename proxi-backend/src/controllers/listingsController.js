...
const createListing = async (req, res) => {
  const { title, description, category, price, price_type, quantity_available, condition, delivery_available, location_lat, location_lng, area_description, images, cover_image, delivery_fee, negotiate } = req.body;
  try {
    ...
    const newListing = await pool.query(
      ...
      [
        req.user.id, title, description || null, category, price,
        price_type || "fixed", quantity_available || 1,
        condition || "used_good", delivery_available || false,
...
const getListings = async (req, res) => {
  ...
  try {
    let query = `SELECT l.*, u.full_name as seller_name, u.proxi_score as seller_score, u.score_tier as seller_tier 
    ...
    if (area) {
      params.push(`%${area}%`);
      query += ` AND l.area_description ILIKE $${params.length}`;
    }
    if (search) {
      params.push(`%${search}%`);
...
const getListing = async (req, res) => {
  try {
    const listing = await pool.query(
      `SELECT l.*, u.full_name as seller_name, u.proxi_score as seller_score, u.score_tier as seller_tier, u.phone_number as seller_phone 
       FROM listings l JOIN users u ON l.seller_id = u.id WHERE l.id = $1 AND l.status = 'active'`,
      [req.params.id]
    );
